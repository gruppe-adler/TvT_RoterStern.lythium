...
[{
    ...
    if ({_side = _x; ({side _x == _side} count (playableUnits + switchableUnits)) > 0} count _sides == 0) exitWith {

        [{
            params ["_winName","_sides","_taskID"];
            if ({_side = _x; ({side _x == _side} count (playableUnits + switchableUnits)) > 0} count _sides == 0) then {
